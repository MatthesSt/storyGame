...
function checkPortals() {
  ...
  const usedPortal = areas.value[currentArea.value].portals.find(
    (p) => p.position[0] == playerTileIndices[0] && p.position[1] == playerTileIndices[1]
  )
  if (usedPortal && !usedPortal.blocked) {
    currentArea.value = usedPortal.targetArea
    const nextPortal = areas.value[currentArea.value].portals.find((p) => p.id == usedPortal.targetPortalId)
...
export function sellItem(itemId: number, merchant: Entity | undefined) {
  if (itemId == 0 || !merchant || !merchant.talking || !merchant.inventory.openend) return
...
export function hasFreeInventorySpace(entitiy: Entity, itemId: number) {
  return (
    ...
    entitiy.inventory.items.find((i) => i.id == itemId)?.amount! < items[itemId].maxStack!
  )
}
